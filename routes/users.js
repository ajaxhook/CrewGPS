...
// util: procura por email/nome sem diferenciar maiúsculas/minúsculas
...
router.get('/me', auth, async (req, res) => {
  try {
    ...
    // normaliza campos esperados no frontend
    me.garage = me.garage || [];
 ...
 * body: { profilePicture?, nome? }
 */
router.put('/profile', auth, async (req, res) => {
  try {
    const updates = {};
    if (typeof req.body.nome === 'string' && req.body.nome.trim()) {
      // garantir nome único (case-insensitive, exclui o próprio)
...
router.delete('/me', auth, async (req, res) => {
  try {
    ...
    // remove de listas de amigos de outros
    await User.updateMany({ friends: userId }, { $pull: { friends: userId } });

    // limpa possíveis pedidos de amizade (se o modelo existir)
    try {
      const FriendRequest = require('../models/FriendRequest');
      await FriendRequest.deleteMany({ $or: [{ from: userId }, { to: userId }] });
    } catch (_) {
      // se não existir FriendRequest, ignora
    }
 ...
 * retorna: [{ _id, nome, profilePicture }]
 */
